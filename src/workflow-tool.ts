...
export function createWorkflowTool(options: WorkflowToolOptions = {}): ToolDefinition<typeof workflowToolSchema, any> {
  return defineTool({
    ...
    promptGuidelines: [
      ...
      `## Model selection`,
      "",
      `Use explicit \`model\` refs. Combined with \`thinkingLevel\` for effort control. Match model capability to task complexity:`,
      `- Cheap scans, grep, classification, fan-out: deepseek-v4-flash, anthropic-haiku, gpt-mini`,
      `- Code generation, review, implementation: deepseek-v4-pro, anthropic-sonnet, kimi-2.5/2.6`,
      ...
      `  const data = await agent("scan", { label: "scan", model: "deepseek-v4-flash" })`,
      `  const ref = handoff(data)  // sync, returns string`,
      `  await agent("analyze: " + ref, { label: "analyze", model: "deepseek-v4-pro" })`,
      ``,
      ...
      "",
      `For multi-agent teams with directed communication, use spawn() + mailbox inside a phase body:`,
      `  const arch = spawn("Design the interface", { label: "architect", mailbox: true, model: "gpt-5.5" })`,
      `  const worker = spawn("Implement from design", { label: "worker", mailbox: true, model: "deepseek-v4-pro", tools: ["edit", "write"] })`,
      `  mailbox.connect(arch.id, worker.id)`,
      `  await parallel([() => arch.result, () => worker.result])`,
      `Mailbox-enabled agents receive mailbox_peers, mailbox_send, and mailbox_pause tools. Channel permissions are one-way (mailbox.allow makes A->B; mailbox.connect makes bidirectional). Always await all spawn handles before the body returns — leaked handles fail the workflow.`,
      "",
      `# Terminology (use these terms consistently)`,
      `- Subagent: an agent spawned inside a workflow via agent() or spawn()`,
      `- Model: explicit model-id string (e.g. "deepseek-v4-flash"). Combine with thinkingLevel for effort`,
      ...
      '  return await agent("Create a detailed plan", { label: "plan", model: "gpt-5.5", thinkingLevel: "high" })',
      "})",
      "",
      ...
      '      label: "impl-" + mod, model: "deepseek-v4-pro",',
      ...
      '      label: "run-tests", model: "deepseek-v4-flash", tools: ["bash"],',
      "    })",
