...
export function createWorkflowTool(options: WorkflowToolOptions = {}): ToolDefinition<typeof workflowToolSchema, any> {
  return defineTool({
    ...
    promptGuidelines: [
      ...
      `## pipeline()`,
      "",
      `Run each item through sequential stages while items fan out concurrently. Signature: \`pipeline(items, stage1, stage2, ...)\` — the first argument is the items array, then each stage as a SEPARATE argument (NOT a single array of stages). Each stage receives \`(previousStageResult, originalItem, index)\`. Failed items return null.`,
      `Example:`,
      `  const results = await pipeline(`,
      `    files,`,
      `    (prev, file, index) => agent("Analyze " + file, { label: "analyze " + file, model: "provider/fast-model" }),`,
      ...
      `Prefer supervisor-seeded starts for simple mailbox flows: spawn the agents, connect them, then use \`await mailbox.send(worker.id, "initial contract")\` so at least one paused agent is guaranteed to wake. Use peer mailbox_send only for bounded replies/notifications, not open-ended negotiation loops.`,
      `  const arch = spawn("Design the interface; if you receive an ack, summarize it, otherwise finish after your design", { label: "architect", mailbox: true, model: "provider/reasoning-model" })`,
      `  const worker = spawn("Wait for one supervisor contract message, send one acknowledgement to architect, then finish", { label: "worker", mailbox: true, model: "provider/code-model", tools: ["edit", "write"] })`,
      `  mailbox.connect(arch.id, worker.id)`,
      `  await mailbox.send(worker.id, "Initial contract: ...")`,
      `  await parallel([() => arch.result, () => worker.result])`,
      `Mailbox-enabled agents receive mailbox_peers, mailbox_send, and mailbox_pause tools. Channel permissions are one-way (mailbox.allow makes A->B; mailbox.connect makes bidirectional). Agents that send to peers should call mailbox_peers first to discover peer ids. Always await all spawn handles before the body returns — leaked handles fail the workflow. If a mailbox team may wait, set a short policy.mailboxPauseTimeoutSeconds and include an explicit fallback.`,
      "",
